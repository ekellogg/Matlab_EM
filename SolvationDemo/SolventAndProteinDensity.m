function [compositeMap proteinMap]=SolventAndProteinDensity(atomCoords, atomTypes)
...
% density map compositeMat which including simulation of solvent.  The
...
% Use the DownsampeGeneral function to change the voxel size post hoc:
...
% Check to see if the single argument is a filename
if nargin==1 && nd==1 && ischar(atomCoords)  % a single string; must be a filename
    pdbName=atomCoords;
...
a=atomCoords(:,q);       % coords of valid entries
...
mincoord=min(a,[],2);  % this is vector of the minimum coordinates
maxcoord=max(a,[],2);
ctrcoord=(mincoord+maxcoord)/2;
span=max(maxcoord-mincoord);  % largest span

boundary=15;  % number of A of boundary
n=ceil(span+2*boundary)*gridfactor;
a=(a+n/2+1-repmat(ctrcoord,1,na))*gridfactor;  % center the coordinates in our box.
...
voln(indexc)=-2.65;   %-2.65 is the value which makes EM value caused by solvent smaller than 0.0001.
